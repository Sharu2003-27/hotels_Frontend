import React, { useState } from "react"
 ...
 const AddHotelForm = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch("https://be-4-4-hw-2-hotels-be.vercel.app/hotels",
             {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                throw "Failed to add new hotel."
            }
            const data = await response.json()
            console.log("Added new hotel:", data);
        } catch (error) {
            console.log(error);
